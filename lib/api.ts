import type { User, Listing, Booking, ListingFilters, SignUpData, BookingData, ListingData } from "@/types"
import {
  mockUsers,
  mockListings,
  getFeaturedListings,
...
export async function getListings(filters?: ListingFilters): Promise<Listing[]> {
  ...
  return mockListings.filter((listing) => {
    ...
    if (filters.minTrustScore > 0 && listing.minTrustScore < filters.minTrustScore) {
      return false
    }

    if (filters.amenities && filters.amenities.length > 0) {
      return filters.amenities.every((amenity) => listing.amenities.includes(amenity))
    }
...
export async function mockCreateBooking(data: BookingData): Promise<Booking> {
  ...
  const newBooking: Booking = {
    ...
    checkIn: data.checkIn.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }),
    checkOut: data.checkOut.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }),
    guests: data.guests,
